...
func (txgroup *Transactions) SetExpire(cfg *ChainConfig, n int, expire time.Duration) {
	if n >= len(txgroup.GetTxs()) {
		return
	}
...
//IsExpire 交易是否过期
func (txgroup *Transactions) IsExpire(cfg *ChainConfig, height, blocktime int64) bool {
	txs := txgroup.Txs
	for i := 0; i < len(txs); i++ {
...
func (txgroup *Transactions) CheckWithFork(cfg *ChainConfig, checkFork, paraFork bool, height, minfee, maxFee int64) error {
	txs := txgroup.Txs
	if len(txs) < 2 {
...
func (txgroup *Transactions) Check(cfg *ChainConfig, height, minfee, maxFee int64) error {
...
func (tx *TransactionCache) Check(cfg *ChainConfig, height, minfee, maxFee int64) error {
	if !tx.checked {
		tx.checked = true
...
//Check 交易检测
func (tx *Transaction) Check(cfg *ChainConfig, height, minfee, maxFee int64) error {
	group, err := tx.GetTxGroup()
	if err != nil {
		return err
	}
...
//SetExpire 设置交易过期时间
func (tx *Transaction) SetExpire(cfg *ChainConfig, expire time.Duration) {
	//Txheight处理
...
//IsExpire 交易是否过期
func (tx *Transaction) IsExpire(cfg *ChainConfig, height, blocktime int64) bool {
...
//GetTxHeight 获取交易高度
func GetTxHeight(cfg *ChainConfig, valid int64, height int64) int64 {
	if cfg.IsPara() {
		return -1
	}
...
func (tx *Transaction) GetEthTxHash() []byte {
	...
	var evmaction EVMContractAction4Chain
