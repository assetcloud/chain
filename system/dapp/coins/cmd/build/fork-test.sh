...
function sendcoinsTx() {
    ...
    #组合形式交易
    #hash=$(sudo docker exec -it $1 ./chain-cli coins transfer -t $2 -a $5 -n $4 | tr '\r' ' ')
    #echo $hash
    #sign=$(sudo docker exec -it $1 ./chain-cli wallet sign -a $3 -d $hash | tr '\r' ' ')
    #echo $sign
    #sudo docker exec -it $1 ./chain-cli wallet send -d $sign

    #单个命令形式交易
    #sudo docker exec -it $1 ./chain-cli send coins transfer -a $5 -n $4 -t $2 -k $From
