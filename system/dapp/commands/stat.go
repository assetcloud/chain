...
func totalCoins(cmd *cobra.Command, args []string) {
	...
	if actual != "" {
		...
		for count = 100; count == 100; {
			...
			err = rpc.Call("Chain.GetTotalCoins", &params, &res)
			if err != nil {
				fmt.Fprintln(os.Stderr, err)
				return
			}
...
func execBalance(cmd *cobra.Command, args []string) {
	...
	for {
		...
		var str string
		err = rpc.Call("Chain.GetExecBalance", &reqParam, &str)
		if err != nil {
			fmt.Fprintln(os.Stderr, err)
			return
		}
...
func getLastBlock(rpc *jsonclient.JSONClient) (*rpctypes.Header, error) {
	...
	res := &rpctypes.Header{}
	err := rpc.Call("Chain.GetLastHeader", nil, &res)
	if err != nil {
		return nil, err
	}
	return res, nil
...
func getBlockHash(height int64, rpc *jsonclient.JSONClient) (string, error) {
	...
	err := rpc.Call("Chain.GetBlockHash", &params, &res)
	if err != nil {
		return "", err
	}
	return res.Hash, nil
...
func queryTotalFeeWithHash(blockHash string, rpc *jsonclient.JSONClient) (*types.TotalFee, error) {
	...
	res := &types.TotalFee{}
	err = rpc.Call("Chain.QueryTotalFee", &params, &res)
	if err != nil {
		return nil, err
	}
	return res, nil
...
func getBlocks(start, end int64, rpc *jsonclient.JSONClient) (*rpctypes.BlockDetails, error) {
	...
	err := rpc.Call("Chain.GetBlocks", params, &res)
	if err != nil {
		return nil, err
	}
	return res, nil
