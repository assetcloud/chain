...
import (
	...
	"github.com/assetcloud/chain/system/dapp/commands/types"
...
func listMempoolTxs(cmd *cobra.Command, args []string) {
	...
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Chain.GetMempool", params, &res)
...
func lastMempoolTxs(cmd *cobra.Command, args []string) {
	rpcLaddr, _ := cmd.Flags().GetString("rpc_laddr")
	var res rpctypes.ReplyTxList
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Chain.GetLastMemPool", nil, &res)
...
func properFee(cmd *cobra.Command, args []string) {
	...
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Chain.GetProperFee", nil, &res)
	ctx.SetResultCb(nil)
	ctx.Run()
}
