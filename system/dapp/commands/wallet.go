...
import (
	...
	ctype "github.com/assetcloud/chain/system/dapp/commands/types"
...
func lock(cmd *cobra.Command, args []string) {
	...
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Chain.Lock", nil, &res)
	ctx.Run()
}
...
func unLock(cmd *cobra.Command, args []string) {
	...
	var res rpctypes.Reply
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Chain.UnLock", &params, &res)
	ctx.Run()
}
...
func walletStatus(cmd *cobra.Command, args []string) {
	...
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Chain.GetWalletStatus", nil, &res)
	ctx.Run()
}
...
func setPwd(cmd *cobra.Command, args []string) {
	...
	var res rpctypes.Reply
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Chain.SetPasswd", &params, &res)
	ctx.Run()
}
...
func walletListTxs(cmd *cobra.Command, args []string) {
	...
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Chain.WalletTxList", params, &res)
...
func mergeBalance(cmd *cobra.Command, args []string) {
	...
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Chain.MergeBalance", &params, &res)
	ctx.Run()
}
...
func noBalanceTx(cmd *cobra.Command, args []string) {
	...
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Chain.CreateNoBalanceTransaction", &params, nil)
...
func signRawTx(cmd *cobra.Command, args []string) {
	...
	if !isFileExist(data) {
		...
		ctx := jsonclient.NewRPCCtx(rpcLaddr, "Chain.SignRawTx", &params, nil)
		ctx.RunWithoutMarshal()
		return
	}
...
func signTxsInFile(req *types.ReqSignRawTx, in, out, rpc string) {
	...
	for line, rawTx := range rawTxs {
		...
		req.TxHex = rawTx
		ctx := jsonclient.NewRPCCtx(rpc, "Chain.SignRawTx", req, &signTx)
		_, err := ctx.RunResult()
		if err != nil {
...
func setFee(cmd *cobra.Command, args []string) {
	...
	var res rpctypes.Reply
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Chain.SetTxFee", &params, &res)
	ctx.Run()
}
...
func sendTxsInFile(req *rpctypes.RawParm, in, out, rpc string) {
	...
	for line, tx := range signTxs {
		...
		req.Data = tx
		ctx := jsonclient.NewRPCCtx(rpc, "Chain.SendTransaction", req, &hash)
		_, err := ctx.RunResult()
		if err != nil {
