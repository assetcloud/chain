...
func genSeed(cmd *cobra.Command, args []string) {
	...
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Chain.GenSeed", &params, &res)
	_, err := ctx.RunResult()
	if err != nil {
		fmt.Println(err)
		return
...
func getSeed(cmd *cobra.Command, args []string) {
	...
	var res types.ReplySeed
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Chain.GetSeed", &params, &res)
	ctx.Run()
}
...
func saveSeed(cmd *cobra.Command, args []string) {
	...
	ctx := jsonclient.NewRPCCtx(rpcLaddr, "Chain.SaveSeed", &params, &res)
	ctx.Run()
	//钱包解锁60s
	...
	jsonclient.NewRPCCtx(rpcLaddr, "Chain.UnLock", &params1, &res1)
}
