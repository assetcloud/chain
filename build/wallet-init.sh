...
function pub2priv() {
    ...
    note=$4
    amount=$5
    #sudo docker exec -it $name ./chain-cli privacy pub2priv -f $fromAdd -p $priAdd -a $amount -n $note
...
function init() {
    ...
    result=$(./chain-cli seed save -p 1314fuzamei -s "tortoise main civil member grace happy century convince father cage beach hip maid merry rib" | jq ".isok")
    ...
    echo "=========== # unlock wallet ============="
    result=$(./chain-cli wallet unlock -p 1314fuzamei -t 0 | jq ".isok")
    if [ "${result}" = "false" ]; then
        exit 1
    fi
    ...
    result=$(./chain-cli account import_key -k CC38546E9E659D15E6B4893F0AB32A06D103931A8230B0BDE71459D2B27D6944 -l transfer | jq ".label")
    echo "${result}"
    if [ -z "${result}" ]; then
        exit 1
    fi
    ...
    result=$(./chain-cli account import_key -k 4257D8692EF7FE13C68B65D6A52F03933DB2FA5CE8FAF210B5B8B80C721CED01 -l mining | jq ".label")
    echo "${result}"
    if [ -z "${result}" ]; then
        exit 1
    fi
    ...
    result=$(./chain-cli wallet auto_mine -f 1 | jq ".isok")
    if [ "${result}" = "false" ]; then
        exit 1
    fi
