...
func (up *updateInitStrategy) Run() error {
	mlog.Info("Begin run chain update init.go.")
	defer mlog.Info("Run chain update init.go finish.")
...
func (up *updateInitStrategy) initMember() error {
	...
	if err != nil || path == "" {
		if len(gopath) > 0 {
			path = filepath.Join(gopath, "/src/github.com/assetcloud/chain/plugin/")
		}
	}
	if packname == "" {
	...
	if len(path) == 0 {
		return errors.New("Chain Plugin Not Existed")
	}
